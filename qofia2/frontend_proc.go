...
func dispatchEvent(evto *thspbs.Event) {
	...
	case "ConferencePeerName":
		...
		oldPeerCount := len(dm.vtcli.Binfo.GetGroupMembers(gnum))
		dm.vtcli.Binfo.UpdatePeerInfo(gnum, groupId, peerPubkey, peerName, pnum)
		_ = oldPeerCount
		/*
			item := uictx.iteman.Get(groupId)
			if item != nil {
	...
	case "ConferencePeerListChange":
		...
		*/
		...
		for _, pubkey := range deletedPeerPubkeys {
			dm.vtcli.Binfo.DeletePeerInfo(gnum, groupId, pubkey)
		}
